...
-- Enhanced Notification System (Based on Starlight patterns)
function UltimateUI:Notify(options)
    ...
    -- Create notification with better styling
    local notifGui = Instance.new("ScreenGui")
    notifGui.Name = "UltimateUI_Notification"
    ...
    frame.Position = UDim2.new(1, 400, 0, 20)  -- Start off-screen
    ...
    iconGlow.Image = self:GetThemeColor("Glow")
    iconGlow.ImageColor3 = self:GetThemeColor("Accent")
    ...
    contentLabel.Size = UDim2.new(1, -70, 0, 45)
    ...
    progressBg.BackgroundColor3 = self:GetThemeColor("Tertiary")
    ...
    self:AddThemeObject(iconGlow, {ImageColor3 = "Accent"})
    self:AddThemeObject(titleLabel, {TextColor3 = "Text"})
    self:AddThemeObject(contentLabel, {TextColor3 = "TextSecondary"})
    self:AddThemeObject(progressBg, {BackgroundColor3 = "Tertiary"})
    self:AddThemeObject(progressBar, {BackgroundColor3 = "Accent"})
    
    -- Enhanced animations
    CreateTween(frame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, -400, 0, 20)
    })
    
    CreateTween(iconGlow, TweenInfo.new(0.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
...
-- Enhanced Dragging System (Based on Atonium patterns)
function UltimateUI:MakeDraggable(object, handle)
    ...
    self:AddConnection(handle.InputBegan:Connect(function(input)
...
-- Enhanced Mobile Button (Based on Atonium patterns)
function UltimateUI:CreateMobileButton()
    if not self.IsMobile then return end
    
    local mobileButton = Instance.new("TextButton")
    mobileButton.Name = "UltimateUI_MobileButton"
    mobileButton.Size = UDim2.new(0, 70, 0, 70)
    mobileButton.Position = UDim2.new(1, -90, 1, -90)
    mobileButton.BackgroundColor3 = self:GetThemeColor("Secondary")
    mobileButton.BorderSizePixel = 0
    mobileButton.Text = ""
    mobileButton.ZIndex = 100
    mobileButton.Parent = self.ScreenGui
    ...
    icon.Image = self.Icons.mobile
    icon.ImageColor3 = self:GetThemeColor("Text")
    icon.Parent = mobileButton
    
    -- State text
    local stateText = Instance.new("TextLabel")
    stateText.Size = UDim2.new(0, 60, 0, 20)
    stateText.Position = UDim2.new(0.5, 0, 1, 10)
    stateText.AnchorPoint = Vector2.new(0.5, 0)
    stateText.BackgroundTransparency = 1
    stateText.Text = self.WindowVisible and "Close" or "Open"
    stateText.TextColor3 = self:GetThemeColor("Text")
    stateText.TextSize = 12
    stateText.Font = Enum.Font.GothamBold
    stateText.Parent = mobileButton
    
    -- Theme objects
    self:AddThemeObject(mobileButton, {BackgroundColor3 = "Secondary"})
    self:AddThemeObject(icon, {ImageColor3 = "Text"})
    self:AddThemeObject(stateText, {TextColor3 = "Text"})
    
    -- Enhanced interaction
    local function updateButtonState()
        stateText.Text = self.WindowVisible and "Close" or "Open"
        CreateTween(icon, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
            Rotation = self.WindowVisible and 45 or 0
        })
    end
    
    -- Button functionality
    self:AddConnection(mobileButton.MouseButton1Click:Connect(function()
        self:ToggleWindow()
        updateButtonState()
    end))
    
    -- Touch feedback
    self:AddConnection(mobileButton.MouseButton1Down:Connect(function()
        CreateTween(mobileButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {
            Size = UDim2.new(0, 65, 0, 65)
        })
    end))
    
    self:AddConnection(mobileButton.MouseButton1Up:Connect(function()
        CreateTween(mobileButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {
...
function UltimateUI:ToggleWindow()
    ...
    if self.WindowVisible then
        ...
        CreateTween(self.MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = self.OriginalSize,
            Position = UDim2.new(0.5, 0, 0.5, 0)
        })
    else
        CreateTween(self.MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
...
-- Enhanced Window Creation (Based on all source library patterns)
function UltimateUI:CreateWindow(options)
    options = options or {}
    local title = options.Title or "UltimateUI"
    local size = options.Size or (self.IsMobile and UDim2.new(0, 400, 0, 500) or UDim2.new(0, 650, 0, 450))
    ...
    -- Main window frame with enhanced styling
    ...
    minimizeBtn.BackgroundColor3 = self:GetThemeColor("Tertiary")
    ...
    self:AddThemeObject(minimizeBtn, {BackgroundColor3 = "Tertiary", TextColor3 = "Text"})
    self:AddThemeObject(closeBtn, {BackgroundColor3 = "Error"})
    
    -- Tab system (Enhanced from source libraries)
    ...
    tabList.Size = UDim2.new(1, 0, 1, -15)
    tabList.Position = UDim2.new(0, 0, 0, 15)
    ...
    local function addButtonHover(button, normalColor, hoverColor)
        self:AddConnection(button.MouseEnter:Connect(function()
            CreateTween(button, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
                BackgroundColor3 = hoverColor or self:GetThemeColor("Accent")
            })
        end))
        
        self:AddConnection(button.MouseLeave:Connect(function()
            CreateTween(button, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
                BackgroundColor3 = normalColor or self:GetThemeColor("Tertiary")
            })
        end))
    end
    
    addButtonHover(minimizeBtn, self:GetThemeColor("Tertiary"), self:GetThemeColor("Accent"))
    addButtonHover(closeBtn, self:GetThemeColor("Error"), Color3.fromRGB(220, 50, 47))
    ...
    function Window:CreateTab(options)
        ...
        local layoutOrder = options.LayoutOrder or (self.TabCount + 1)
        
        self.TabCount = self.TabCount + 1
        
        -- Tab button (Enhanced from Atonium patterns)
        local tabButton = Instance.new("TextButton")
        tabButton.Name = "Tab_" .. name
        tabButton.Size = UDim2.new(1, 0, 0, self.IsMobile and 50 or 42)
        tabButton.BackgroundColor3 = UltimateUI:GetThemeColor("Tertiary")
        tabButton.BorderSizePixel = 0
        tabButton.Text = ""
        tabButton.LayoutOrder = layoutOrder
        tabButton.Parent = UltimateUI.CurrentWindow.TabList
        
        local tabCorner = Instance.new("UICorner")
        tabCorner.CornerRadius = UDim.new(0, 10)
        tabCorner.Parent = tabButton
        
        -- Tab fill (for selection state)
        ...
        tabGlow.Name = "Glow"
        tabGlow.Size = UDim2.new(0, 52, 0, 52)
        tabGlow.Position = UDim2.new(0.5, 0, 0.5, 0)
        tabGlow.AnchorPoint = Vector2.new(0.5, 0.5)
        tabGlow.BackgroundTransparency = 1
        tabGlow.Image = UltimateUI:GetThemeColor("Glow")
        tabGlow.ImageColor3 = UltimateUI:GetThemeColor("Accent")
        ...
        tabContent.Parent = UltimateUI.CurrentWindow.ContentContainer
        ...
        -- Theme objects
        UltimateUI:AddThemeObject(tabButton, {BackgroundColor3 = "Tertiary"})
        ...
        function Tab:Select()
            ...
            for _, otherTab in pairs(Window.Tabs) do
                if otherTab ~= self and otherTab.Active then
                    otherTab:Deselect()
                end
            end
            ...
            CreateTween(self.Icon, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
                ImageColor3 = UltimateUI:GetThemeColor("Text")
            })
            
            CreateTween(self.Text, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
                TextColor3 = UltimateUI:GetThemeColor("Text")
            })
        end
        ...
        -- Tab click handler
        UltimateUI:AddConnection(tabButton.MouseButton1Click:Connect(function()
            Tab:Select()
        end))
        ...
        -- Enhanced Component Creation Methods (Based on source library patterns)
        ...
        function Tab:CreateToggle(options)
            ...
            -- Click handler
            UltimateUI:AddConnection(toggleFrame.MouseButton1Click:Connect(function()
                Toggle:Set(not Toggle.State)
            end))
            
            -- Touch support for mobile
            if UltimateUI.TouchSupport and mobileOptimized then
                UltimateUI:AddConnection(toggleFrame.TouchTap:Connect(function()
                    Toggle:Set(not Toggle.State)
                end))
            end
        ...
        function Tab:CreateButton(options)
            ...
            UltimateUI:AddConnection(buttonFrame.MouseButton1Click:Connect(function()
                -- Click animation
            ...
            UltimateUI:AddConnection(sliderTrack.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or
                   (UltimateUI.TouchSupport and input.UserInputType == Enum.UserInputType.Touch) then
                    dragging = true
                    UltimateUI.SliderDrag = true
                    Slider:UpdateFromMouse()
                    
                    input.Changed:Connect(function()
                        if input.UserInputState == Enum.UserInputState.End then
                            dragging = false
                            UltimateUI.SliderDrag = false
                        end
                    end)
                end
            end))
    ...
    UltimateUI.CurrentWindow = Window
    return Window
end
